...
local function githubRead(path)
    ...
    local ok, resp = pcall(function()
        return HttpService:RequestAsync({Url=url,Method="GET",Headers={["Authorization"]="token "..CFG.GITHUB_TOKEN,["Accept"]="application/vnd.github.v3.raw"}})
    end)
...
local function githubWrite(path, content, message)
    ...
    local shaOk, shaResp = pcall(function()
        return HttpService:RequestAsync({Url=shaUrl,Method="GET",Headers={["Authorization"]="token "..CFG.GITHUB_TOKEN,["Accept"]="application/vnd.github.v3+json",["User-Agent"]="Susano-Script"}})
    end)
    local currentSHA = ""
    ...
    local body = HttpService:JSONEncode({message=message or "update",content=b64encode(content),sha=currentSHA~="" and currentSHA or nil,branch=CFG.GITHUB_BRANCH})
    local ok, resp = pcall(function()
        return HttpService:RequestAsync({Url=shaUrl,Method="PUT",Headers={["Authorization"]="token "..CFG.GITHUB_TOKEN,["Accept"]="application/vnd.github.v3+json",["Content-Type"]="application/json",["User-Agent"]="Susano-Script"},Body=body})
    end)
...
local function gistCreate(name, content)
    local body = HttpService:JSONEncode({description="Susano Config: "..name,public=false,files={[name..".json"]={content=content}}})
    local ok, resp = pcall(function()
        return HttpService:RequestAsync({Url="https://api.github.com/gists",Method="POST",Headers={["Authorization"]="token "..CFG.GIST_TOKEN,["Accept"]="application/vnd.github.v3+json",["Content-Type"]="application/json",["User-Agent"]="Susano-Script"},Body=body})
    end)
    if not ok or not resp.Success then return false, nil end
...
local function gistRead(gistId)
    local ok, resp = pcall(function()
        return HttpService:RequestAsync({Url="https://api.github.com/gists/"..gistId,Method="GET",Headers={["Authorization"]="token "..CFG.GIST_TOKEN,["Accept"]="application/vnd.github.v3+json",["User-Agent"]="Susano-Script"}})
    end)
    if not ok or not resp.Success then return false, nil end
    local ok2, data = pcall(function() return HttpService:JSONDecode(resp.Body) end)
    ...
    for _, file in pairs(data.files) do
        if file.content then
            ...
            if ok3 and cfg then return true, cfg end
        end
    end
    return false, nil
end
...
local function gistDelete(gistId)
    local ok, resp = pcall(function()
        return HttpService:RequestAsync({Url="https://api.github.com/gists/"..gistId,Method="DELETE",Headers={["Authorization"]="token "..CFG.GIST_TOKEN,["User-Agent"]="Susano-Script"}})
    end)
...
-- ========== THEME ==========
local T={
    BG=Color3.fromRGB(12,12,12),Sidebar=Color3.fromRGB(16,16,16),
    Card=Color3.fromRGB(26,26,26),CardHov=Color3.fromRGB(34,34,34),
    Accent=Color3.fromRGB(255,255,255),AccentDim=Color3.fromRGB(180,180,180),
    AccentFaint=Color3.fromRGB(48,48,48),TitleBar=Color3.fromRGB(10,10,10),
    ...
    MinBtn=Color3.fromRGB(42,42,42),ActiveSide=Color3.fromRGB(245,245,245),
    InactiveSide=Color3.fromRGB(26,26,26),
    KeyGreen=Color3.fromRGB(30,180,80),KeyRed=Color3.fromRGB(200,50,50),
    KeyGold=Color3.fromRGB(220,180,50),Online=Color3.fromRGB(40,200,100),
}
...
local function buildKeyMenu(onSuccess)
    ...
    for i=0,30 do
        local l=Instance.new("Frame",overlay);l.Size=UDim2.new(1,0,0,1)
    ...
    topBar.BackgroundColor3=T.TitleBar;topBar.BorderSizePixel=0;topBar.ClipsDescendants=true;corner(12,topBar)
    local tbF=Instance.new("Frame",topBar);tbF.Size=UDim2.new(1,0,0.5,0);tbF.Position=UDim2.new(0,0,0.5,0);tbF.BackgroundColor3=T.TitleBar;tbF.BorderSizePixel=0
    local tL=Instance.new("TextLabel",topBar);tL.Size=UDim2.new(1,0,0.65,0);tL.BackgroundTransparency=1;tL.Text="SUSANO";tL.TextColor3=T.Accent;tL.Font=Enum.Font.GothamBlack;tL.TextSize=24
    local sL=Instance.new("TextLabel",topBar);sL.Size=UDim2.new(1,0,0.4,0);sL.Position=UDim2.new(0,0,0.62,0);sL.BackgroundTransparency=1;sL.Text="v1.0  —  Anahtarını gir";sL.TextColor3=T.TextFaint;sL.Font=Enum.Font.GothamMedium;sL.TextSize=11
    ...
    pcall(function()
        local img=Instance.new("ImageLabel",avF);img.Size=UDim2.new(1,0,1,0);img.BackgroundTransparency=1;corner(24,img)
    ...
    inputBox.BackgroundColor3=T.Card;inputBox.TextColor3=T.Text;inputBox.Font=Enum.Font.GothamMedium;inputBox.TextSize=15
    inputBox.PlaceholderText="SUSANO-XXXX-XXXXXXXX";inputBox.PlaceholderColor3=T.TextFaint;inputBox.Text="";inputBox.ClearTextOnFocus=false;inputBox.BorderSizePixel=0
    ...
    remBtn.MouseButton1Click:Connect(function()
        remOn=not remOn;tw(remPill,0.18,{BackgroundColor3=remOn and T.OnBG or T.OffBG})
...
-- ========== CROSSHAIR ==========
local chGui;local CROSSHAIR_STYLES={"Cross","Circle","Dot","T-Shape","X-Shape","Square"}
...
tabBuilders["TEAM CHANGE"]=function(p)
    ...
    cBtn.MouseButton1Click:Connect(function() if uBox.Text=="" or tBox.Text=="" then res.Text="Tüm alanları doldur!";res.TextColor3=T.KeyRed;return end;local ok,msg=changeTeam(uBox.Text,tBox.Text);res.Text=msg;res.TextColor3=ok and T.KeyGreen or T.KeyRed;_G.TeamChangePlayerName=uBox.Text;_G.TeamChangeTeamName=tBox.Text end)
end
...
tabBuilders["CONFIG"]=function(p)
    ...
    saveBtn.MouseButton1Click:Connect(function()
        local n=nameBox.Text:gsub("[^%w%-_]",""):lower();if n=="" then res.Text="Config adı gir!";res.TextColor3=T.KeyRed;return end
    ...
    loadBtn.MouseButton1Click:Connect(function()
        local gistId=codeBox.Text:gsub("%s+","");if gistId=="" then loadRes.Text="Gist ID gir!";loadRes.TextColor3=T.KeyRed;return end
        ...
        if ok and data then applyConfigData(data);buildCrosshair();loadRes.Text="Config yüklendi!"..(data.savedBy and (" ("..data.savedBy..")") or "");loadRes.TextColor3=T.KeyGreen
        else loadRes.Text="Yükleme başarısız.";loadRes.TextColor3=T.KeyRed end
    end)
    y=buildSection(sc,"Kayıtlı Configler",y+4);local gmap=loadGistMap();local hasConfig=false
    ...
    if not hasConfig then local el=Instance.new("TextLabel",sc);el.Size=UDim2.new(1,-28,0,28);el.Position=UDim2.new(0,14,0,y);el.BackgroundTransparency=1;el.Text="Henüz config kaydedilmedi.";el.TextColor3=T.TextFaint;el.Font=Enum.Font.GothamMedium;el.TextSize=13;el.TextXAlignment=Enum.TextXAlignment.Left end
end
tabBuilders["ABOUT"]=function(p)
    ...
    for _,row in ipairs({{"Kısayol","F5 — Menü Aç/Kapat"},{"Key Sistemi","GitHub Private Repo. HWID kilitleme aktif."},{"Config","Gist ID ile paylaş."},{"OBS Bypass","DisplayOrder 200."},{"Respawn","Özellikler ölünce kapanmaz."}}) do
        ...
        local fp=Instance.new("UIPadding",card);fp.PaddingBottom=UDim.new(0,10);y=y+78
    end
end
...
local function createMenu()
    ...
    local tb=Instance.new("Frame",MainFrame);tb.Size=UDim2.new(1,0,0,46);tb.BackgroundColor3=T.TitleBar;tb.BorderSizePixel=0;tb.ClipsDescendants=true;local tbF=Instance.new("Frame",tb);tbF.Size=UDim2.new(1,0,0.5,0);tbF.Position=UDim2.new(0,0,0.5,0);tbF.BackgroundColor3=T.TitleBar;tbF.BorderSizePixel=0;corner(10,tb)
    ...
    SideBar=Instance.new("Frame",MainFrame);SideBar.Size=UDim2.new(0,170,1,-46);SideBar.Position=UDim2.new(0,0,0,46);SideBar.BackgroundColor3=T.Sidebar;SideBar.BorderSizePixel=0;local sbF=Instance.new("Frame",SideBar);sbF.Size=UDim2.new(1,0,0.5,0);sbF.BackgroundColor3=T.Sidebar;sbF.BorderSizePixel=0;corner(10,SideBar)
