...
local function githubRead(path)
    local url = string.format(
        ...
        CFG.GITHUB_OWNER, CFG.GITHUB_REPO, CFG.GITHUB_BRANCH, path
    )
...
local function makeGui(name, order)
    local e=CoreGui:FindFirstChild(name);if e then e:Destroy() end
    local g=Instance.new("ScreenGui");g.Name=name;g.Parent=CoreGui
    g.ResetOnSpawn=false;g.IgnoreGuiInset=true;g.DisplayOrder=order or 200
...
local function buildKeyMenu(onSuccess)
    ...
    local sL=Instance.new("TextLabel",topBar);sL.Size=UDim2.new(1,0,0.4,0)
    ...
    avF.Position=UDim2.new(0,10,0.5,-24);avF.BackgroundColor3=T.AccentFaint;corner(24,avF)
    pcall(function()
        local img=Instance.new("ImageLabel",avF);img.Size=UDim2.new(1,0,1,0)
        img.BackgroundTransparency=1;corner(24,img)
    ...
    for _,bd in ipairs({
        ...
        local bl=Instance.new("TextLabel",b);bl.Size=UDim2.new(1,0,1,0)
    ...
    remLbl.BackgroundTransparency=1;remLbl.Text="Bu cihazda anahtarı hatırla"
    remLbl.TextColor3=T.TextDim;remLbl.Font=Enum.Font.GothamMedium;remLbl.TextSize=13
    remLbl.TextXAlignment=Enum.TextXAlignment.Left
    local remOn=true
    local remPill=Instance.new("Frame",remRow);remPill.Size=UDim2.new(0,44,0,22)
    remPill.Position=UDim2.new(1,-44,0.5,-11);remPill.BackgroundColor3=T.OnBG;corner(22,remPill)
    local remKnob=Instance.new("Frame",remPill);remKnob.Size=UDim2.new(0,16,0,16)
    remKnob.Position=UDim2.new(1,-19,0.5,-8);remKnob.BackgroundColor3=T.TitleBar;corner(100,remKnob)
    local remBtn=Instance.new("TextButton",remRow);remBtn.Size=UDim2.new(1,0,1,0)
    remBtn.BackgroundTransparency=1;remBtn.Text=""
    ...
    activateBtn.Font=Enum.Font.GothamBold;activateBtn.TextSize=16;activateBtn.Text="GİRİŞ YAP"
    ...
    local function tryActivate()
        ...
        validateKey(key, function(ok, result)
            ...
            if ok then
                ...
                tw(ibStroke,0.2,{Color=T.KeyGreen})
                task.wait(0.7)
                ...
                tw(ibStroke,0.1,{Color=T.KeyRed})
...
local function createFOVCircle()
    ...
    local r=_G.FOVSize;FOVCircle.Size=UDim2.new(0,r*2,0,r*2)
    FOVCircle.Position=UDim2.new(0.5,-r,0.5,-r);FOVCircle.BackgroundTransparency=1
    FOVCircle.BorderSizePixel=0;FOVCircle.ZIndex=999;corner(999,FOVCircle)
    local s=Instance.new("UIStroke",FOVCircle);s.Color=Color3.new(1,1,1)
    s.Thickness=1;s.Transparency=0.45;s.Name="FOVStroke"
    FOVCircle.Visible=_G.FOVVisible
end
local function updateFOVColor(col)
    if FOVCircle then local s=FOVCircle:FindFirstChild("FOVStroke");if s then s.Color=col end end
end
...
buildCrosshair=function()
    ...
    local function mkLine(w,h,ox,oy)
        ...
        f.Size=UDim2.new(0,w,0,h);f.AnchorPoint=Vector2.new(0.5,0.5)
        f.Position=UDim2.new(0.5,ox,0.5,oy);f.ZIndex=500
        if _G.CrosshairOutline then
            local os=Instance.new("UIStroke",f);os.Color=Color3.new(0,0,0)
            os.Thickness=1;os.Transparency=alpha+0.3
        end;return f
    end
...
local function buildConfigData()
    return {
        ...
        Aimbot=_G.Aimbot,RageAimbot=_G.RageAimbot,SilentAim=_G.SilentAim,AutoShoot=_G.AutoShoot,UseFOV=_G.UseFOV,FOVVisible=_G.FOVVisible,FOVSize=_G.FOVSize,AimbotSmoothness=_G.AimbotSmoothness,AimbotPartHead=_G.AimbotPartHead,AimbotPartChest=_G.AimbotPartChest,AimbotPartStomach=_G.AimbotPartStomach,
        FlyEnabled=_G.FlyEnabled,FlySpeed=_G.FlySpeed,NoClipEnabled=_G.NoClipEnabled,BunnyHop=_G.BunnyHop,BunnyHopSpeed=_G.BunnyHopSpeed,BunnyHopHeight=_G.BunnyHopHeight,SpeedHack=_G.SpeedHack,SpeedMultiplier=_G.SpeedMultiplier,InfiniteJump=_G.InfiniteJump,LongJump=_G.LongJump,LongJumpPower=_G.LongJumpPower,SwimHack=_G.SwimHack,
...
tabBuilders["TEAM CHANGE"]=function(p)
    local sc=makeScroll(p);local y=10;y=buildSection(sc,"Takım Değiştir",y)
    local uBox;uBox,y=buildInput(sc,"Kullanıcı","İsim",_G.TeamChangePlayerName,y);local tBox;tBox,y=buildInput(sc,"Takım","Takım adı",_G.TeamChangeTeamName,y);local cBtn;cBtn,y=buildButton(sc,"Değiştir",y)
...
tabBuilders["CONFIG"]=function(p)
    ...
    if not hasConfig then
        local el=Instance.new("TextLabel",sc);el.Size=UDim2.new(1,-28,0,28);el.Position=UDim2.new(0,14,0,y);el.BackgroundTransparency=1;el.Text="Henüz config kaydedilmedi.";el.TextColor3=T.TextFaint;el.Font=Enum.Font.GothamMedium;el.TextSize=13;el.TextXAlignment=Enum.TextXAlignment.Left
    end
end
...
switchTab=function(id)
    ...
    for _,btn in ipairs(sideButtons) do
        ...
        tw(btn,0.15,{BackgroundColor3=active and T.ActiveSide or T.InactiveSide})
...
local function createMenu()
    ...
    local closeBtn=mkTBtn("x",T.CloseRed,-36)
    closeBtn.MouseEnter:Connect(function() tw(closeBtn,0.1,{BackgroundColor3=Color3.fromRGB(220,70,70)}) end)
    closeBtn.MouseLeave:Connect(function() tw(closeBtn,0.1,{BackgroundColor3=T.CloseRed}) end)
    ...
    local minBtn=mkTBtn("-",T.MinBtn,-70)
    ...
    for i,tab in ipairs(TABS) do
        ...
        btn.BackgroundColor3=tab.id==currentTab and T.ActiveSide or T.InactiveSide;btn.AutoButtonColor=false;btn.BorderSizePixel=0;btn.Text="";corner(7,btn);btn:SetAttribute("tabId",tab.id)
