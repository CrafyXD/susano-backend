...
local function formatTimeLeft(expires)
    if expires == 0 or expires == math.huge then return "Sınırsız" end
    local remaining = expires - os.time()
    if remaining <= 0 then return "Süresi Doldu" end
    local days = math.floor(remaining / 86400)
    local hours = math.floor((remaining % 86400) / 3600)
    return string.format("%dg %02dsa", days, hours)
end
...
local function validateKey(key, callback)
    ...
    if key == "" then 
        if callback then callback(false, "Please enter a key", 0) end
        return false, "Please enter a key"
    end
    
    local kd = VALID_KEYS[key]
    if not kd then
        if callback then callback(false, "Invalid key", 0) end
        return false, "Invalid key"
    end
    
    -- Süre kontrolü
    if kd.expires and kd.expires < os.time() then
        if callback then callback(false, "Key expired", 0) end
        return false, "Key expired"
    end
    
    if callback then callback(true, kd.keyType or "lifetime", kd.expires or 0) end
    return true, kd.keyType or "lifetime"
end

-- Kayıtlı key'i yükle
local function loadSavedKey()
    local ok, content = safeRead(SAVE_PATH)
...
if savedKey then
    ...
    if ok then
        ...
        pcall(function()
            if keyMenuGui and keyMenuGui.Parent then
                keyMenuGui:Destroy()
            end
        end)
