-- SUSANO V2.2 FIXED & BYPASS EDITION
-- Tum hatalar giderildi, AC bypass eklendi.
...
-- DÜZELTİLMİŞ HTTP REQUEST (Nil Value Hatasını Önler)
local function hReq(m,u,b,h)
    local reqFunc = http_request or request or (syn and syn.request)
    if not reqFunc then return false, nil end -- Eğer request yoksa false döndür, crash ettirme
...
task.spawn(function() task.wait(3);pcall(blockACRemotes) end)
...
task.spawn(function() task.wait(3);pcall(scanRemotes) end)
...
local function mkWarn(parent,txt,yPos)
    local warn=Instance.new("TextLabel",parent)
    warn.Size=UDim2.new(1,-28,0,30);warn.Position=UDim2.new(0,14,0,yPos)
    warn.BackgroundColor3=Color3.fromRGB(60,20,10);warn.BackgroundTransparency=0
    warn.Text=txt;warn.TextColor3=Color3.fromRGB(255,100,80)
    warn.Font=Enum.Font.GothamBold;warn.TextSize=11;warn.TextWrapped=true
    corner(6,warn);return yPos+38
end
        ...
        if input.UserInputType==Enum.UserInputType.MouseButton2 then
            if tpPos then
                pcall(function()
                    ...
                    task.delay(0.3,function() pcall(function() tpInd.BackgroundColor3=Color3.fromRGB(100,220,255) end) end)
                end)
            end
        end
    end)
...
-- Fake Lag
local flConn
local function eFL() if flConn then flConn:Disconnect() end;flConn=RunSvc.Heartbeat:Connect(function() if not _G.FL then return end;task.wait(_G.FLI) end) end
...
dGD=function()
    if gdC1 then gdC1:Disconnect();gdC1=nil end
    if gdC2 then gdC2:Disconnect();gdC2=nil end
    pcall(function()
        local char=LP.Character;if not char then return end
        local hum=char:FindFirstChildOfClass("Humanoid");if not hum then return end
...
-- ============ SPEED HACK - ANTICHEAT BYPASS (FIXED) ============
...
dKA=function() if kaC then kaC:Disconnect();kaC=nil end end
...
dAFK=function() if afkC then afkC:Disconnect();afkC=nil end end
...
apNS=function()
    ...
    if _G.NS then nsC=LP.CharacterAdded:Connect(function(char) task.wait(0.5);pcall(function() spoof(char) end) end) end
end
...
local function setupARej()
    Players.PlayerRemoving:Connect(function(p)
        if p==LP and _G.ARJ then task.wait(3);pcall(function() game:GetService("TeleportService"):Teleport(game.PlaceId,LP) end) end
    end)
end
...
local function eFPS()
    if fpsA then return end;fpsA=true
    pcall(function()
        for _,obj in ipairs(WS:GetDescendants()) do
            if obj:IsA("ParticleEmitter") or obj:IsA("Smoke") or obj:IsA("Fire") or obj:IsA("Sparkles") then
                pcall(function() obj.Enabled=false;table.insert(hidP,obj) end)
            end
        end
...
local function dFPS()
    fpsA=false
    for _,obj in ipairs(hidP) do pcall(function() obj.Enabled=true end) end
    hidP={};pcall(function() LT.GlobalShadows=true end)
end
...
local function dCL() if clC then clC:Disconnect();clC=nil end;if clGui then clGui:Destroy();clGui=nil;clScFrame=nil end end
...
-- ============ MENU BUILD (Eksik olan kısım) ============
local tabs = {"Movement", "Combat", "Visuals", "Misc", "AntiCheat"}
...
rebuildMenu = function()
    ...
    local MainFrame = Instance.new("Frame", MGui)
    ...
    Header.BackgroundColor3 = Tc.TB
    Header.BorderSizePixel = 0;corner(12, Header)
    local hc=Instance.new("UICorner",Header);hc.CornerRadius=UDim.new(0,12)
    local fix=Instance.new("Frame",Header);fix.Size=UDim2.new(1,0,0.5,0);fix.Position=UDim2.new(0,0,1,-10);fix.BackgroundColor3=Tc.TB;fix.BorderSizePixel=0
    
    local TitleL = Instance.new("TextLabel", Header)
    TitleL.Size = UDim2.new(0, 200, 1, 0);TitleL.Position = UDim2.new(0, 15, 0, 0)
    TitleL.BackgroundTransparency = 1;TitleL.Text = "SUSANO V2.2"
    TitleL.TextColor3 = Tc.AC;TitleL.Font = Enum.Font.GothamBold;TitleL.TextSize = 20
    ...
    TabList.FillDirection = Enum.FillDirection.Horizontal;TabList.Padding = UDim.new(0, 8)
    
    for _, tabName in ipairs(tabs) do
        local TabBtn = Instance.new("TextButton", TabBar)
        TabBtn.Size = UDim2.new(0, 80, 1, 0)
        TabBtn.BackgroundColor3 = (currentTab == tabName) and Tc.Card or Tc.CardH
        TabBtn.Text = tabName;TabBtn.TextColor3 = Tc.Txt
        TabBtn.Font = Enum.Font.GothamBold;TabBtn.TextSize = 12
        TabBtn.BorderSizePixel = 0;corner(6, TabBtn)
    ...
    if currentTab == "Movement" then
        yPos = mkSec(Content, "Fly", yPos)
        yPos = mkToggle(Content, T("t_fly"), "FLY", yPos, function(v) if v then eFly() else dFly() end end)
        yPos = mkSlider(Content, T("lfls"), "FLS", yPos, 0, 500, "%.0f", function(v) end)
        yPos = mkWarn(Content, "⚠️ Çok fazla hız ban yedirebilir!", yPos)
        
        yPos = mkSec(Content, "Speed", yPos)
        yPos = mkToggle(Content, T("t_sh"), "SP", yPos, function(v) if v then eSP() else dSP() end end)
        yPos = mkSlider(Content, T("lspd"), "SPM", yPos, 1, 100, "%.1fx", function(v) end)
        yPos = mkWarn(Content, "⚠️ Speed hack sunucuda anlaşılabilir!", yPos)
        
        yPos = mkSec(Content, "Other", yPos)
        yPos = mkToggle(Content, T("t_nc"), "NC", yPos, function(v) if v then eNC() else dNC() end end)
        yPos = mkToggle(Content, T("t_bh"), "BH", yPos, function(v) if v then eBH() else dBH() end end)
        yPos = mkToggle(Content, T("t_ij"), "IJ", yPos, function(v) if v then eIJ() else dIJ() end end)
        yPos = mkToggle(Content, T("t_lj"), "LJ", yPos, function(v) if v then eLJ() else dLJ() end end)
        
    elseif currentTab == "AntiCheat" then
        yPos = mkSec(Content, "Bypass Systems", yPos)
        yPos = mkToggle(Content, T("t_acfl"), "ACFly", yPos, function(v) end)
        yPos = mkToggle(Content, T("t_acsp"), "ACSp", yPos, function(v) end)
        yPos = mkToggle(Content, T("t_acre"), "ACRe", yPos, function(v) end)
        yPos = mkWarn(Content, "Bypasslar açıkken bile şüpheli hareketler ban yedirebilir.", yPos)
    end
end
    ...
    pcall(function()
        ...
        m.KeyDown:Connect(function(k)
            if k=="p" then
                ...
                if MGui then MGui.Enabled=menuOpen end
            end
        end)
    end)
    rebuildMenu()
