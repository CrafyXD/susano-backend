--[[
    ...
    KEY MENU OPENS INSTANTLY ON LOAD
    F5 blocked until key is validated
--]]
...
local setnetworkowner   = setnetworkowner or function(part,plr)
...
-- ========== KEY DATABASE ==========
-- Add/remove keys here. expires=math.huge = never expires.
-- To add time-limited keys: expires = os.time() + seconds
-- Daily  = os.time() + 86400
-- Weekly = os.time() + 604800
-- Monthly= os.time() + 2592000
local VALID_KEYS = {
    ["SUSANO-DAILY-DEMO1"]  = {keyType="daily",    expires=math.huge},
    ["SUSANO-WEEK-DEMO1"]   = {keyType="weekly",   expires=math.huge},
    ["SUSANO-MONTH-DEMO1"]  = {keyType="monthly",  expires=math.huge},
    ["SUSANO-LIFE-DEMO1"]   = {keyType="lifetime", expires=math.huge},
}

-- ========== FILE HELPERS ==========
local SAVE_PATH   = "susano_key.txt"
local CONFIG_PATH = "susano_configs/"
...
-- ========== KEY VALIDATION ==========
-- Bu kodu susano_main.lua'daki validateKey fonksiyonuyla değiştir

-- Webhook URL - kendi Discord webhook URL'ini buraya yaz
local WEBHOOK_URL = "https://discord.com/api/webhooks/1486036090472108123/4cWWaTLIYICUV741VkYoxGoT3LTAu70qxZOniTpsOCXkVTwyOGXG8hKZqLDG5kLLBqzp"
...
local function validateKey(key, callback)
    ...
    task.spawn(function()
        ...
        if not kd then
            -- Webhook: Başarısız giriş
            sendWebhook("Geçersiz Key Denemesi", 15158332, {
                { name = "Kullanıcı", value = USERNAME, inline = true },
                { name = "HWID", value = HWID:sub(1,20), inline = true },
                { name = "Key", value = key:sub(1,20).."...", inline = false },
                { name = "Oyun", value = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name or tostring(game.PlaceId), inline = false },
            })
            callback(false, T("key_invalid"), 0); return
        end

        -- Key süresi: sadece kullanılmışsa kontrol et
        if kd.activated and kd.expires and kd.expires > 0 and os.time() > kd.expires then
            callback(false, T("key_expired"), 0); return
        end

        -- HWID kontrolü: key daha önce kullanıldıysa
        if kd.activated and kd.hwid and kd.hwid ~= "" and tostring(kd.hwid) ~= "null" then
            if tostring(kd.hwid) ~= HWID then
                -- Webhook: Farklı HWID denemesi
                sendWebhook("Yetkisiz HWID Denemesi", 15158332, {
                    { name = "Kullanıcı", value = USERNAME, inline = true },
                    { name = "HWID (Girilen)", value = HWID:sub(1,20), inline = true },
                    { name = "HWID (Kayıtlı)", value = tostring(kd.hwid):sub(1,20), inline = true },
                    { name = "Key Sahibi", value = key:sub(1,12).."...", inline = false },
                })
                callback(false, T("key_other_device"), 0); return
            end
        else
            -- İlk kullanım: HWID bağla ve süreyi ŞIMDI başlat
            keysData[key].hwid = HWID
            keysData[key].activated = true

            -- Süreyi şimdi başlat (duration > 0 ise)
            local duration = kd.duration or 0
            if duration > 0 then
                keysData[key].expires = os.time() + duration
            else
                keysData[key].expires = 0 -- lifetime
            end

            -- GitHub'a kaydet
            task.spawn(function()
                local ok, json = pcall(function() return HttpService:JSONEncode(keysData) end)
                if ok then githubWrite("keys.json", json, "hwid+expire bind: "..USERNAME) end
            end)

            -- Webhook: Yeni aktivasyon
            local typeNames = {daily="Günlük", weekly="Haftalık", monthly="Aylık", lifetime="Lifetime"}
            sendWebhook("Yeni Key Aktivasyonu", 3066993, {
                { name = "Kullanıcı", value = USERNAME.." ("..USER_ID..")", inline = true },
                { name = "Key Türü", value = typeNames[kd.type] or kd.type, inline = true },
                { name = "HWID", value = HWID:sub(1,30), inline = false },
                { name = "Oyun", value = tostring(game.PlaceId), inline = true },
                { name = "Sunucu", value = tostring(game.JobId):sub(1,20), inline = true },
                { name = "Süre Bitiş", value = keysData[key].expires == 0 and "Sınırsız" or os.date("%d.%m.%Y %H:%M", keysData[key].expires), inline = false },
            })
        end

        -- Webhook: Başarılı giriş (zaten aktive edilmiş key)
        if kd.activated then
            sendWebhook("Key Girişi", 3447003, {
                { name = "Kullanıcı", value = USERNAME.." ("..USER_ID..")", inline = true },
                { name = "Key Türü", value = kd.type or "?", inline = true },
                { name = "HWID", value = HWID:sub(1,30), inline = false },
                { name = "Oyun", value = tostring(game.PlaceId), inline = true },
                { name = "Kalan Süre", value = formatTimeLeft(kd.expires or 0), inline = true },
            })
        end
...
_G.Verified = false  -- BLOCKED until key validated
...
_G.TeamCheck=false;_G.ShowFriendly=false;_G.WallCheck=false;_G.SkeletonESP=false
...
_G.TimeChanger=false;_G.TimeOfDay=14
_G.WorldColor=false;_G.WorldR=128;_G.WorldG=128;_G.WorldB=128
...
local T={
    ...
    AccentFaint = Color3.fromRGB(48,48,48),
    ...
    ActiveSide  = Color3.fromRGB(245,245,245),
    InactiveSide= Color3.fromRGB(26,26,26),
...
local function tw(obj,t,props) TweenService:Create(obj,TweenInfo.new(t,Enum.EasingStyle.Quad),props):Play() end
...
local function makeGui(name, displayOrder)
    ...
    gui.DisplayOrder = displayOrder or 200
    pcall(function() gui.ZIndexBehavior = Enum.ZIndexBehavior.Global end)
    return gui
end

-- =====================================================
-- ========== KEY MENU — OPENS IMMEDIATELY ==========
-- =====================================================
local keyMenuGui = makeGui("SusanoKeyMenu", 999) -- Highest priority
...
local function buildKeyMenu(onSuccess)
    ...
    -- Subtle scanlines
    for i=0,30 do
        local l=Instance.new("Frame",overlay)
        l.Size=UDim2.new(1,0,0,1)
        l.Position=UDim2.new(0,0,0,i*36)
    ...
    card.Size = UDim2.new(0,460,0,500)
    card.Position = UDim2.new(0.5,-230,0.5,-250)
    ...
    for _,sd in ipairs({{4,3,0.82},{12,6,0.88},{22,10,0.94}}) do
        local s=Instance.new("Frame",card); s.Size=UDim2.new(1,sd[1],1,sd[1]); s.Position=UDim2.new(0,-sd[1]/2,0,sd[2])
        s.BackgroundColor3=Color3.new(0,0,0); s.BackgroundTransparency=sd[3]; s.ZIndex=card.ZIndex-1; s.BorderSizePixel=0; corner(15,s)
    end

    -- Top bar
    local topBar=Instance.new("Frame",card); topBar.Size=UDim2.new(1,0,0,52); topBar.BackgroundColor3=T.TitleBar; topBar.BorderSizePixel=0; topBar.ClipsDescendants=true
    local tbF=Instance.new("Frame",topBar); tbF.Size=UDim2.new(1,0,0.5,0); tbF.Position=UDim2.new(0,0,0.5,0); tbF.BackgroundColor3=T.TitleBar; tbF.BorderSizePixel=0; corner(12,topBar)
    local titleLbl=Instance.new("TextLabel",topBar); titleLbl.Size=UDim2.new(1,0,0.65,0); titleLbl.BackgroundTransparency=1; titleLbl.Text="SUSANO"; titleLbl.TextColor3=T.Accent; titleLbl.Font=Enum.Font.GothamBlack; titleLbl.TextSize=24
    local subLbl=Instance.new("TextLabel",topBar); subLbl.Size=UDim2.new(1,0,0.4,0); subLbl.Position=UDim2.new(0,0,0.62,0); subLbl.BackgroundTransparency=1; subLbl.Text="v1.0  —  Enter your key to continue"; subLbl.TextColor3=T.TextFaint; subLbl.Font=Enum.Font.GothamMedium; subLbl.TextSize=11

    -- Profile card
    local profBg=Instance.new("Frame",card); profBg.Size=UDim2.new(1,-32,0,74); profBg.Position=UDim2.new(0,16,0,68); profBg.BackgroundColor3=T.Card; corner(10,profBg)

    -- Avatar
    local avFrame=Instance.new("Frame",profBg); avFrame.Size=UDim2.new(0,50,0,50); avFrame.Position=UDim2.new(0,12,0.5,-25); avFrame.BackgroundColor3=T.AccentFaint; corner(25,avFrame)
    pcall(function()
        local img=Instance.new("ImageLabel",avFrame); img.Size=UDim2.new(1,0,1,0); img.BackgroundTransparency=1; corner(25,img)
        img.Image=Players:GetUserThumbnailAsync(LocalPlayer.UserId,Enum.ThumbnailType.HeadShot,Enum.ThumbnailSize.Size150x150)
    end)

    local info=Instance.new("Frame",profBg); info.Size=UDim2.new(1,-76,1,0); info.Position=UDim2.new(0,70,0,0); info.BackgroundTransparency=1
    local un=Instance.new("TextLabel",info); un.Size=UDim2.new(1,0,0,26); un.Position=UDim2.new(0,0,0,10); un.BackgroundTransparency=1; un.Text="@"..USERNAME; un.TextColor3=T.Text; un.Font=Enum.Font.GothamBold; un.TextSize=15; un.TextXAlignment=Enum.TextXAlignment.Left
    local hw=Instance.new("TextLabel",info); hw.Size=UDim2.new(1,0,0,18); hw.Position=UDim2.new(0,0,0,34); hw.BackgroundTransparency=1; hw.Text="HWID: "..HWID:sub(1,22)..".."; hw.TextColor3=T.TextFaint; hw.Font=Enum.Font.GothamMedium; hw.TextSize=11; hw.TextXAlignment=Enum.TextXAlignment.Left
    local uid=Instance.new("TextLabel",info); uid.Size=UDim2.new(1,0,0,18); uid.Position=UDim2.new(0,0,0,50); uid.BackgroundTransparency=1; uid.Text="UserID: "..USER_ID.."  |  PlaceID: "..tostring(game.PlaceId); uid.TextColor3=T.TextFaint; uid.Font=Enum.Font.GothamMedium; uid.TextSize=10; uid.TextXAlignment=Enum.TextXAlignment.Left

    -- Key type badges
    local badgesRow=Instance.new("Frame",card); badgesRow.Size=UDim2.new(1,-32,0,30); badgesRow.Position=UDim2.new(0,16,0,152); badgesRow.BackgroundTransparency=1
    local badgeLayout=Instance.new("UIListLayout",badgesRow); badgeLayout.FillDirection=Enum.FillDirection.Horizontal; badgeLayout.Padding=UDim.new(0,6); badgeLayout.VerticalAlignment=Enum.VerticalAlignment.Center
    local bDefs={{l="DAILY",c=T.TextDim},{l="WEEKLY",c=Color3.fromRGB(80,180,255)},{l="MONTHLY",c=Color3.fromRGB(180,120,255)},{l="LIFETIME",c=T.KeyGold}}
    for _,bd in ipairs(bDefs) do
        local b=Instance.new("Frame",badgesRow); b.Size=UDim2.new(0,84,1,0); b.BackgroundColor3=T.Card; corner(5,b)
        local bl=Instance.new("TextLabel",b); bl.Size=UDim2.new(1,0,1,0); bl.BackgroundTransparency=1; bl.Text=bd.l; bl.TextColor3=bd.c; bl.Font=Enum.Font.GothamBold; bl.TextSize=11
    end

    -- Input label
    local inputLbl=Instance.new("TextLabel",card); inputLbl.Size=UDim2.new(1,-32,0,20); inputLbl.Position=UDim2.new(0,16,0,194); inputLbl.BackgroundTransparency=1; inputLbl.Text="KEY"; inputLbl.TextColor3=T.TextFaint; inputLbl.Font=Enum.Font.GothamBold; inputLbl.TextSize=10; inputLbl.TextXAlignment=Enum.TextXAlignment.Left

    -- Input box
    local inputBox=Instance.new("TextBox",card)
    inputBox.Size=UDim2.new(1,-32,0,46); inputBox.Position=UDim2.new(0,16,0,216)
    inputBox.BackgroundColor3=T.Card; inputBox.TextColor3=T.Text
    inputBox.Font=Enum.Font.GothamMedium; inputBox.TextSize=15
    inputBox.PlaceholderText="SUSANO-XXXX-XXXXXXXX"; inputBox.PlaceholderColor3=T.TextFaint
    inputBox.Text=""; inputBox.ClearTextOnFocus=false; inputBox.BorderSizePixel=0
    corner(8,inputBox); pad(inputBox,14,0)
    local ib_stroke=Instance.new("UIStroke",inputBox); ib_stroke.Color=T.BorderLight; ib_stroke.Thickness=1

    -- Status
    local statusLbl=Instance.new("TextLabel",card); statusLbl.Size=UDim2.new(1,-32,0,22); statusLbl.Position=UDim2.new(0,16,0,268); statusLbl.BackgroundTransparency=1; statusLbl.Text=""; statusLbl.Font=Enum.Font.GothamMedium; statusLbl.TextSize=13; statusLbl.TextXAlignment=Enum.TextXAlignment.Left

    -- Remember toggle
    local remRow=Instance.new("Frame",card); remRow.Size=UDim2.new(1,-32,0,30); remRow.Position=UDim2.new(0,16,0,296); remRow.BackgroundTransparency=1
    local remLbl=Instance.new("TextLabel",remRow); remLbl.Size=UDim2.new(0.75,0,1,0); remLbl.BackgroundTransparency=1; remLbl.Text="Remember key on this device"; remLbl.TextColor3=T.TextDim; remLbl.Font=Enum.Font.GothamMedium; remLbl.TextSize=13; remLbl.TextXAlignment=Enum.TextXAlignment.Left
    local rememberOn=true
    ...
    remBtn.MouseButton1Click:Connect(function()
        ...
        tw(remPill,0.18,{BackgroundColor3=rememberOn and T.OnBG or T.OffBG})
        tw(remKnob,0.18,{Position=UDim2.new(rememberOn and 1 or 0,rememberOn and -19 or 3,0.5,-8),BackgroundColor3=rememberOn and T.TitleBar or T.AccentDim})
    end)

    -- Activate button
    local activateBtn=Instance.new("TextButton",card)
    activateBtn.Size=UDim2.new(1,-32,0,46); activateBtn.Position=UDim2.new(0,16,0,336)
    activateBtn.BackgroundColor3=T.Accent; activateBtn.TextColor3=T.BG
    activateBtn.Font=Enum.Font.GothamBold; activateBtn.TextSize=16; activateBtn.Text="ACTIVATE"
    ...
    -- Discord hint
    local discLbl=Instance.new("TextLabel",card); discLbl.Size=UDim2.new(1,-32,0,20); discLbl.Position=UDim2.new(0,16,0,390); discLbl.BackgroundTransparency=1; discLbl.Text="Get a key from our Discord server"; discLbl.TextColor3=T.TextFaint; discLbl.Font=Enum.Font.GothamMedium; discLbl.TextSize=12

    -- Version footer
    local footLbl=Instance.new("TextLabel",card); footLbl.Size=UDim2.new(1,-32,0,20); footLbl.Position=UDim2.new(0,16,0,414); footLbl.BackgroundTransparency=1; footLbl.Text="Susano V1.0  |  Build 2025"; footLbl.TextColor3=T.TextFaint; footLbl.Font=Enum.Font.GothamMedium; footLbl.TextSize=11
    ...
    local function tryActivate()
        ...
        if key=="" then statusLbl.Text="Please enter a key."; statusLbl.TextColor3=T.KeyRed; return end

        -- Flash border
        tw(ib_stroke,0.1,{Color=T.AccentDim})

        local ok,result=validateKey(key)
        if ok then
            if rememberOn then safeWrite(SAVE_PATH,key) end

            -- Success animation
            statusLbl.Text="Key valid!  Type: "..result:upper()
            statusLbl.TextColor3=T.KeyGreen
            tw(activateBtn,0.2,{BackgroundColor3=T.KeyGreen})
            tw(ib_stroke,0.2,{Color=T.KeyGreen})
            task.wait(0.7)

            -- Fade out
            tw(card,0.25,{BackgroundTransparency=1})
            ...
            attempts=attempts+1
            statusLbl.Text="Invalid key  ("..attempts.." attempt".. (attempts>1 and "s" or "")..")"
            statusLbl.TextColor3=T.KeyRed
            tw(ib_stroke,0.1,{Color=T.KeyRed})
            tw(inputBox,0.08,{BackgroundColor3=Color3.fromRGB(55,18,18)})
            task.wait(0.25)
            tw(inputBox,0.2,{BackgroundColor3=T.Card})
            tw(ib_stroke,0.3,{Color=T.BorderLight})
    ...
    -- Instantly focus the input box
    task.spawn(function()
        task.wait(0.05)
        inputBox:CaptureFocus()
    end)

    -- Card entrance (very fast — 0.18s)
    card.BackgroundTransparency=1
    card.Position=UDim2.new(0.5,-230,0.5,-240)
    tw(card,0.18,{BackgroundTransparency=0, Position=UDim2.new(0.5,-230,0.5,-250)})
end

-- ========== FORWARD DECLARATIONS ==========
local enableFly,disableFly
local enableNoclip,disableNoclip
local enableBhop,disableBhop
local enableSpeed,disableSpeed
local enableInfiniteJump,disableInfiniteJump
local enableLongJump,disableLongJump
local enableSwimHack
local enableKillAura,disableKillAura
local enableAntiAFK,disableAntiAFK
local enableRageAimbot,disableRageAimbot
local enableSilentAim,disableSilentAim
local enableThirdPerson,disableThirdPerson
local applyNameSpoof
local applyFullBright,applyNoFog,applyTime,applyWorldColor
local buildCrosshair
local MainGui, switchTab

-- ========== FOV CIRCLE ==========
local FOVCircle,FOVGui
local function createFOVCircle()
    if FOVGui then FOVGui:Destroy() end
    FOVGui=makeGui("SusanoFOV",200)
    FOVCircle=Instance.new("Frame",FOVGui); FOVCircle.Name="FOVCircle"
    local r=_G.FOVSize; FOVCircle.Size=UDim2.new(0,r*2,0,r*2); FOVCircle.Position=UDim2.new(0.5,-r,0.5,-r)
    FOVCircle.BackgroundTransparency=1; FOVCircle.BorderSizePixel=0; FOVCircle.ZIndex=999
    corner(999,FOVCircle)
...
-- ========== CROSSHAIR ==========
local chGui
local CROSSHAIR_STYLES={"Cross","Circle","Dot","T-Shape","X-Shape","Square"}
...
buildCrosshair=function()
    ...
    local function mkLine(w,h,ox,oy)
        local f=Instance.new("Frame",chGui); f.BackgroundColor3=col; f.BorderSizePixel=0; f.BackgroundTransparency=alpha
        f.Size=UDim2.new(0,w,0,h); f.AnchorPoint=Vector2.new(0.5,0.5); f.Position=UDim2.new(0.5,ox,0.5,oy); f.ZIndex=500
        if _G.CrosshairOutline then local os=Instance.new("UIStroke",f); os.Color=Color3.new(0,0,0); os.Thickness=1; os.Transparency=alpha+0.3 end; return f
    end
    ...
    elseif style=="Dot"     then local d=mkLine(th*3,th*3,0,0);corner(100,d)
    ...
    if _G.CrosshairDot and style~="Dot" then local d=mkLine(th+1,th+1,0,0);corner(100,d) end
end
...
local BONES_R6={{"Head","Torso"},{"Torso","HumanoidRootPart"},{"Torso","Right Arm"},{"Torso","Left Arm"},{"Torso","Right Leg"},{"Torso","Left Leg"}}
...
local function updateSkeleton()
    for _,player in ipairs(Players:GetPlayers()) do
        ...
        local char=player.Character; local boneList=char:FindFirstChild("UpperTorso") and BONES_R15 or BONES_R6
        if not skelDrawings[player] then
            skelDrawings[player]={}
            for _=1,#boneList do local l=Drawing.new("Line");l.Visible=false;l.Thickness=1.5;l.Transparency=0.15;l.Color=Color3.new(1,1,1);table.insert(skelDrawings[player],l) end
        end
        ...
        for i,bone in ipairs(boneList) do
            ...
            if p1 and p2 then
                local sp1,on1=Camera:WorldToViewportPoint(p1.Position); local sp2,on2=Camera:WorldToViewportPoint(p2.Position)
...
local function startAutoShoot()
    ...
    autoShootConn=RunService.Heartbeat:Connect(function()
        ...
        local t=getBestTarget(); if t then pcall(mouse1press); task.wait(0.04); pcall(mouse1release) end
    end)
end
...
enableSilentAim=function()
    ...
    saConn1=RunService.Stepped:Connect(function()
        ...
        if t then _savedCam=Camera.CFrame; Camera.CFrame=CFrame.new(Camera.CFrame.Position,t.part.Position); _saApplied=true else _saApplied=false end
    end)
    saConn2=RunService.RenderStepped:Connect(function()
        if _saApplied and _savedCam then Camera.CFrame=_savedCam; _saApplied=false end
    end)
end
disableSilentAim=function()
    silentAimActive=false; if saConn1 then saConn1:Disconnect() end; if saConn2 then saConn2:Disconnect() end
    if _savedCam then Camera.CFrame=_savedCam end
end
    ...
    rageConn=RunService.RenderStepped:Connect(function()
        ...
        for _,p in ipairs(Players:GetPlayers()) do
            if p~=LocalPlayer and p.Character then
                ...
                if not friendly and not _G.TeamCheck then continue end
                local hum=p.Character:FindFirstChildOfClass("Humanoid"); if not hum or hum.Health<=0 then continue end
                for _,pn in ipairs({"Head","UpperTorso","Torso","HumanoidRootPart"}) do local part=p.Character:FindFirstChild(pn); if part then local d=(part.Position-Camera.CFrame.Position).Magnitude; if d<bDist then bDist=d; best=part end end end
            end
        end
...
enableLongJump=function()
    ...
    ljConn=UserInputService.JumpRequest:Connect(function()
        ...
        if hrp and hum and hum.FloorMaterial~=Enum.Material.Air then
            local bv=Instance.new("BodyVelocity"); bv.Velocity=hrp.CFrame.LookVector*_G.LongJumpPower+Vector3.new(0,30,0); bv.MaxForce=Vector3.new(1e5,1e5,1e5); bv.P=1e4; bv.Parent=hrp; game:GetService("Debris"):AddItem(bv,0.15)
        end
    end)
end
...
enableSwimHack=function()
    ...
    swimConn=RunService.Stepped:Connect(function()
        ...
        local c=LocalPlayer.Character; if not c then return end
        local hum=c:FindFirstChildOfClass("Humanoid"); if hum and hum:GetState()==Enum.HumanoidStateType.Swimming then hum.WalkSpeed=16*_G.SpeedMultiplier end
    end)
end
...
enableFly=function()
    ...
    local c=LocalPlayer.Character; if not c then return end
    local hum=c:FindFirstChildOfClass("Humanoid"); local hrp=c:FindFirstChild("HumanoidRootPart"); if not hum or not hrp then return end
    flying=true
...
enableNoclip=function()
    ...
    ncConn=RunService.Stepped:Connect(function()
        ...
        for _,p in pairs(LocalPlayer.Character:GetDescendants()) do if p:IsA("BasePart") and p.CanCollide then p.CanCollide=false; setnetworkowner(p,LocalPlayer) end end
    end)
end
    ...
    local function spoofChar(char)
        ...
        if _G.NameSpoof then
            if not existing then
                ...
                local lbl=Instance.new("TextLabel",bb); lbl.Size=UDim2.new(1,0,1,0); lbl.BackgroundTransparency=1; lbl.TextColor3=Color3.new(1,1,1); lbl.Font=Enum.Font.GothamBold; lbl.TextSize=17; lbl.TextStrokeTransparency=0.5; lbl.Name="SpoofLbl"
            end
...
local origCamFOV=Camera.FieldOfView; local origTimeOfDay=Lighting.TimeOfDay; local origColorShift=Lighting.ColorShift_Top

applyFullBright=function(v)
    if v then Lighting.Ambient=Color3.new(1,1,1); Lighting.Brightness=2
        for _,e in ipairs(Lighting:GetChildren()) do if e:IsA("BlurEffect") or e:IsA("ColorCorrectionEffect") or e:IsA("SunRaysEffect") or e:IsA("BloomEffect") then e.Enabled=false end end
...
-- ========== PLAYER UTILS ==========
local function changeTeam(pName,tName) local tp; for _,p in ipairs(Players:GetPlayers()) do if p.Name:lower()==pName:lower() then tp=p;break end end; if not tp then return false,"Player not found" end; local tt; for _,t in ipairs(game:GetService("Teams"):GetChildren()) do if t.Name:lower()==tName:lower() then tt=t;break end end; if not tt then return false,"Team not found" end; tp.Team=tt; return true,tp.Name.." -> "..tt.Name end
...
local function giveItem(pName,toolName,count) count=count or 1; local tp; for _,p in ipairs(Players:GetPlayers()) do if p.Name:lower():find(pName:lower()) then tp=p;break end end; if not tp then return false,"Player not found" end; local found; for _,loc in ipairs({ServerStorage,ReplicatedStorage,Workspace}) do local t=loc:FindFirstChild(toolName,true); if t and t:IsA("Tool") then found=t:Clone();break end end; if not found then return false,"Tool not found" end; for i=1,count do local cl=found:Clone(); local bp=tp:FindFirstChild("Backpack"); if bp then cl.Parent=bp elseif tp.Character then cl.Parent=tp.Character end; task.wait(0.1) end; return true,count.."x "..toolName.." -> "..tp.Name end

-- ========== CONFIG SYSTEM ==========
local function getAllConfigs()
    local files=safeList(CONFIG_PATH); local configs={}
    for _,f in ipairs(files) do local name=f:match("([^/\\]+)%.susano$") or f:match("([^/\\]+)%.susano"); if name then table.insert(configs,name) end end
    return configs
end
local function saveConfig(name)
    local data={ESP=_G.ESP,ESPBox3D=_G.ESPBox3D,ESPBox2D=_G.ESPBox2D,ShowNames=_G.ShowNames,ShowDistance=_G.ShowDistance,ShowHealthBar=_G.ShowHealthBar,ShowID=_G.ShowID,ShowTracer=_G.ShowTracer,TracerThick=_G.TracerThick,TeamCheck=_G.TeamCheck,ShowFriendly=_G.ShowFriendly,WallCheck=_G.WallCheck,SkeletonESP=_G.SkeletonESP,ESPEnemyR=_G.ESPEnemyR,ESPEnemyG=_G.ESPEnemyG,ESPEnemyB=_G.ESPEnemyB,ESPFriendR=_G.ESPFriendR,ESPFriendG=_G.ESPFriendG,ESPFriendB=_G.ESPFriendB,ESPVisR=_G.ESPVisR,ESPVisG=_G.ESPVisG,ESPVisB=_G.ESPVisB,Crosshair=_G.Crosshair,CrosshairStyle=_G.CrosshairStyle,CrosshairSize=_G.CrosshairSize,CrosshairThick=_G.CrosshairThick,CrosshairGap=_G.CrosshairGap,CrosshairAlpha=_G.CrosshairAlpha,CrosshairDot=_G.CrosshairDot,CrosshairOutline=_G.CrosshairOutline,CrosshairR=_G.CrosshairR,CrosshairG=_G.CrosshairG,CrosshairB=_G.CrosshairB,Aimbot=_G.Aimbot,RageAimbot=_G.RageAimbot,SilentAim=_G.SilentAim,AutoShoot=_G.AutoShoot,UseFOV=_G.UseFOV,FOVVisible=_G.FOVVisible,FOVSize=_G.FOVSize,AimbotSmoothness=_G.AimbotSmoothness,AimbotPartHead=_G.AimbotPartHead,AimbotPartChest=_G.AimbotPartChest,AimbotPartStomach=_G.AimbotPartStomach,FlyEnabled=_G.FlyEnabled,FlySpeed=_G.FlySpeed,NoClipEnabled=_G.NoClipEnabled,BunnyHop=_G.BunnyHop,BunnyHopSpeed=_G.BunnyHopSpeed,BunnyHopHeight=_G.BunnyHopHeight,SpeedHack=_G.SpeedHack,SpeedMultiplier=_G.SpeedMultiplier,InfiniteJump=_G.InfiniteJump,LongJump=_G.LongJump,LongJumpPower=_G.LongJumpPower,SwimHack=_G.SwimHack,KillAura=_G.KillAura,KillAuraRange=_G.KillAuraRange,AntiAFK=_G.AntiAFK,NameSpoof=_G.NameSpoof,SpoofedName=_G.SpoofedName,FullBright=_G.FullBright,NoFog=_G.NoFog,FOVChanger=_G.FOVChanger,FOVChangerVal=_G.FOVChangerVal,ThirdPerson=_G.ThirdPerson,ThirdPersonDist=_G.ThirdPersonDist,TimeChanger=_G.TimeChanger,TimeOfDay=_G.TimeOfDay,WorldColor=_G.WorldColor,WorldR=_G.WorldR,WorldG=_G.WorldG,WorldB=_G.WorldB,savedBy=USERNAME,savedAt=os.time()}
    local ok,json=pcall(function() return HttpService:JSONEncode(data) end); if not ok then return false end
    safeWrite(CONFIG_PATH..name..".susano",json); return true
end
local function loadConfig(name)
    local ok,content=safeRead(CONFIG_PATH..name..".susano"); if not ok or not content then return false,"Config not found" end
    local ok2,data=pcall(function() return HttpService:JSONDecode(content) end); if not ok2 or not data then return false,"Config corrupted" end
    for k,v in pairs(data) do if k~="savedBy" and k~="savedAt" then _G[k]=v end end
    return true,"Loaded: "..name
end
local function deleteConfig(name) safeDel(CONFIG_PATH..name..".susano"); return true end
...
local function buildToggle(parent,label,setting,yPos,onToggle)
    ...
    hit.MouseButton1Click:Connect(function()
        _G[setting]=not _G[setting];tw(pill,0.18,{BackgroundColor3=_G[setting] and T.OnBG or T.OffBG})
...
local function buildColorPreview(parent,sR,sG,sB,yPos)
    ...
    local lbl=Instance.new("TextLabel",row);lbl.Size=UDim2.new(0.5,0,1,0);lbl.Position=UDim2.new(0,12,0,0);lbl.BackgroundTransparency=1;lbl.Text="Color Preview";lbl.TextColor3=T.TextDim;lbl.Font=Enum.Font.GothamMedium;lbl.TextSize=13;lbl.TextXAlignment=Enum.TextXAlignment.Left
...
tabBuilders["ESP"]=function(p)
    ...
    y=buildSection(sc,"Visibility",y);y=buildToggle(sc,"ESP Active","ESP",y,function(v) if not v then clearAllESP() end end);y=buildToggle(sc,"3D Highlight","ESPBox3D",y);y=buildToggle(sc,"2D Box","ESPBox2D",y);y=buildToggle(sc,"Skeleton ESP","SkeletonESP",y)
    y=buildSection(sc,"Labels",y+4);y=buildToggle(sc,"Show Name","ShowNames",y);y=buildToggle(sc,"Show Distance","ShowDistance",y);y=buildToggle(sc,"Show Health Bar","ShowHealthBar",y);y=buildToggle(sc,"Show Player ID","ShowID",y);y=buildToggle(sc,"Tracers","ShowTracer",y);y=buildSlider(sc,"Tracer Thickness","TracerThick",y,0.5,6,"%.1f")
    y=buildSection(sc,"Filters",y+4);y=buildToggle(sc,"Show Enemies","TeamCheck",y);y=buildToggle(sc,"Show Friendly","ShowFriendly",y);y=buildToggle(sc,"Wall Check Color","WallCheck",y)
    y=buildSection(sc,"Enemy Color",y+4);y=buildColorPreview(sc,"ESPEnemyR","ESPEnemyG","ESPEnemyB",y);y=buildSlider(sc,"Red","ESPEnemyR",y,0,255,nil);y=buildSlider(sc,"Green","ESPEnemyG",y,0,255,nil);y=buildSlider(sc,"Blue","ESPEnemyB",y,0,255,nil)
    y=buildSection(sc,"Friendly Color",y+4);y=buildColorPreview(sc,"ESPFriendR","ESPFriendG","ESPFriendB",y);y=buildSlider(sc,"Red","ESPFriendR",y,0,255,nil);y=buildSlider(sc,"Green","ESPFriendG",y,0,255,nil);y=buildSlider(sc,"Blue","ESPFriendB",y,0,255,nil)
    y=buildSection(sc,"Visible Color",y+4);y=buildColorPreview(sc,"ESPVisR","ESPVisG","ESPVisB",y);y=buildSlider(sc,"Red","ESPVisR",y,0,255,nil);y=buildSlider(sc,"Green","ESPVisG",y,0,255,nil);y=buildSlider(sc,"Blue","ESPVisB",y,0,255,nil)
    y=buildSection(sc,"Crosshair",y+4);y=buildToggle(sc,"Crosshair","Crosshair",y,function() buildCrosshair() end)
    y=buildDropdown(sc,"Style",CROSSHAIR_STYLES,function() return _G.CrosshairStyle end,function(v) _G.CrosshairStyle=v;buildCrosshair() end,y)
    y=buildSlider(sc,"Size","CrosshairSize",y,4,50,nil,function() buildCrosshair() end);y=buildSlider(sc,"Thickness","CrosshairThick",y,1,8,nil,function() buildCrosshair() end);y=buildSlider(sc,"Gap","CrosshairGap",y,0,24,nil,function() buildCrosshair() end);y=buildSlider(sc,"Opacity","CrosshairAlpha",y,0.1,1.0,"%.1f",function() buildCrosshair() end)
    y=buildToggle(sc,"Center Dot","CrosshairDot",y,function() buildCrosshair() end);y=buildToggle(sc,"Outline","CrosshairOutline",y,function() buildCrosshair() end)
    y=buildSection(sc,"Crosshair Color",y+4);y=buildColorPreview(sc,"CrosshairR","CrosshairG","CrosshairB",y);y=buildSlider(sc,"Red","CrosshairR",y,0,255,nil,function() buildCrosshair() end);y=buildSlider(sc,"Green","CrosshairG",y,0,255,nil,function() buildCrosshair() end);y=buildSlider(sc,"Blue","CrosshairB",y,0,255,nil,function() buildCrosshair() end)
end
tabBuilders["AIMBOT"]=function(p)
    ...
    y=buildSection(sc,"Auto Shoot",y+4);y=buildToggle(sc,"Auto Shoot","AutoShoot",y)
    y=buildSection(sc,"FOV",y+4);y=buildToggle(sc,"Enable FOV Filter","UseFOV",y);y=buildToggle(sc,"Show FOV Circle","FOVVisible",y,function(v) if FOVCircle then FOVCircle.Visible=v end end);y=buildSlider(sc,"FOV Radius","FOVSize",y,20,400,nil)
    y=buildSection(sc,"Smoothness",y+4);y=buildSlider(sc,"Smoothness","AimbotSmoothness",y,0.02,1.0,"%.2f")
    y=buildSection(sc,"Target Part",y+4);y=buildToggle(sc,"Head","AimbotPartHead",y);y=buildToggle(sc,"Chest","AimbotPartChest",y);y=buildToggle(sc,"Stomach / HRP","AimbotPartStomach",y)
end
tabBuilders["OTHER"]=function(p)
    local sc=makeScroll(p);local y=10
    y=buildSection(sc,"Fly",y);y=buildToggle(sc,"Fly","FlyEnabled",y,function(v) if v then enableFly() else disableFly() end end);y=buildSlider(sc,"Fly Speed","FlySpeed",y,10,300,nil)
    y=buildSection(sc,"Ground",y+4);y=buildToggle(sc,"Noclip","NoClipEnabled",y,function(v) if v then enableNoclip() else disableNoclip() end end);y=buildToggle(sc,"Infinite Jump","InfiniteJump",y,function(v) if v then enableInfiniteJump() else disableInfiniteJump() end end);y=buildToggle(sc,"Long Jump (Space)","LongJump",y,function(v) if v then enableLongJump() else disableLongJump() end end);y=buildSlider(sc,"Long Jump Power","LongJumpPower",y,20,200,nil);y=buildToggle(sc,"Swim Hack","SwimHack",y)
    y=buildSection(sc,"Bunny Hop",y+4);y=buildToggle(sc,"Bunny Hop","BunnyHop",y,function(v) if v then enableBhop() else disableBhop() end end);y=buildSlider(sc,"Bhop Speed","BunnyHopSpeed",y,1.0,3.0,"%.1f")
    y=buildSection(sc,"Speed",y+4);y=buildToggle(sc,"Speed Hack","SpeedHack",y,function(v) if v then enableSpeed() else disableSpeed() end end);y=buildSlider(sc,"Speed Multiplier","SpeedMultiplier",y,1.0,10.0,"%.1f")
    y=buildSection(sc,"Misc",y+4);y=buildToggle(sc,"Stream Proof (OBS Bypass)","StreamProof",y,function(v) if MainGui then MainGui.DisplayOrder=v and 200 or 10 end end)
end
tabBuilders["PLAYERS"]=function(p)
    local sc=makeScroll(p);local y=10
    y=buildSection(sc,"Combat",y);y=buildToggle(sc,"Kill Aura","KillAura",y,function(v) if v then enableKillAura() else disableKillAura() end end);y=buildSlider(sc,"Kill Aura Range","KillAuraRange",y,5,50,nil)
    y=buildSection(sc,"Utility",y+4);y=buildToggle(sc,"Anti AFK","AntiAFK",y,function(v) if v then enableAntiAFK() else disableAntiAFK() end end)
    y=buildSection(sc,"Name Spoof",y+4);y=buildToggle(sc,"Name Spoof","NameSpoof",y,function() applyNameSpoof() end)
    local nameBox;nameBox,y=buildInput(sc,"Spoofed Name","Enter fake name","",y);local applyBtn;applyBtn,y=buildButton(sc,"Apply Name",y);applyBtn.MouseButton1Click:Connect(function() _G.SpoofedName=nameBox.Text;applyNameSpoof() end)
    y=buildSection(sc,"Player List",y+4);local stopBtn,_=buildButton(sc,"Stop Spectating",y,T.AccentFaint,T.Text);y=y+46;stopBtn.MouseButton1Click:Connect(stopSpectating)
    ...
    local playout=Instance.new("UIListLayout",plist);playout.Padding=UDim.new(0,4);local plpad=Instance.new("UIPadding",plist);plpad.PaddingTop=UDim.new(0,6);plpad.PaddingLeft=UDim.new(0,6);plpad.PaddingRight=UDim.new(0,6)
...
tabBuilders["ITEM"]=function(p)
    local sc=makeScroll(p);local y=10;y=buildSection(sc,"Item Giver",y)
    local pBox;pBox,y=buildInput(sc,"Target Username","PlayerName",LocalPlayer.Name,y);local tBox;tBox,y=buildInput(sc,"Tool Name","Sword, AK47","",y);local cBox;cBox,y=buildInput(sc,"Amount","1","1",y)
    local gBtn;gBtn,y=buildButton(sc,"Give Item",y);local sBtn;sBtn,y=buildButton(sc,"Search Tools",y,T.AccentFaint,T.Text)
    ...
    local tlbl=Instance.new("TextLabel",toolsF);tlbl.Size=UDim2.new(1,0,0,26);tlbl.BackgroundTransparency=1;tlbl.Text="Tool List";tlbl.TextColor3=T.TextDim;tlbl.Font=Enum.Font.GothamBold;tlbl.TextSize=11
    ...
    local function rfr() for _,c in ipairs(tsc:GetChildren()) do if c:IsA("TextButton") then c:Destroy() end end;res.Text="Searching...";task.spawn(function() local found={};for _,loc in ipairs({ServerStorage,ReplicatedStorage,Workspace}) do for _,item in ipairs(loc:GetDescendants()) do if item:IsA("Tool") then found[item.Name]=true end end end;local cnt=0;for name in pairs(found) do local btn=Instance.new("TextButton",tsc);btn.Size=UDim2.new(1,0,0,26);btn.Text=name;btn.TextColor3=T.Text;btn.Font=Enum.Font.Gotham;btn.TextSize=13;btn.BackgroundColor3=T.CardHov;corner(5,btn);btn.MouseButton1Click:Connect(function() tBox.Text=name end);cnt=cnt+1 end;res.Text=cnt>0 and (cnt.." found") or "None found";res.TextColor3=cnt>0 and Color3.fromRGB(80,255,120) or Color3.fromRGB(255,80,80) end) end
    gBtn.MouseButton1Click:Connect(function() local ok,msg=giveItem(pBox.Text,tBox.Text,math.clamp(tonumber(cBox.Text) or 1,1,20));res.Text=msg;res.TextColor3=ok and Color3.fromRGB(80,255,120) or Color3.fromRGB(255,80,80) end);sBtn.MouseButton1Click:Connect(rfr);rfr()
end
tabBuilders["TEAM CHANGE"]=function(p)
    local sc=makeScroll(p);local y=10;y=buildSection(sc,"Team Change",y)
    local uBox;uBox,y=buildInput(sc,"Username","PlayerName",_G.TeamChangePlayerName,y);local tBox;tBox,y=buildInput(sc,"Team Name","TeamName",_G.TeamChangeTeamName,y)
    local cBtn;cBtn,y=buildButton(sc,"Change Team",y);local res=Instance.new("TextLabel",sc);res.Size=UDim2.new(1,-28,0,28);res.Position=UDim2.new(0,14,0,y);res.BackgroundTransparency=1;res.TextColor3=T.TextDim;res.Font=Enum.Font.GothamMedium;res.TextSize=13;res.TextXAlignment=Enum.TextXAlignment.Left
    cBtn.MouseButton1Click:Connect(function() if uBox.Text=="" or tBox.Text=="" then res.Text="Fill all fields!";res.TextColor3=T.KeyRed;return end;local ok,msg=changeTeam(uBox.Text,tBox.Text);res.Text=msg;res.TextColor3=ok and T.KeyGreen or T.KeyRed;_G.TeamChangePlayerName=uBox.Text;_G.TeamChangeTeamName=tBox.Text end)
end
tabBuilders["VISUAL"]=function(p)
    local sc=makeScroll(p);local y=10
    y=buildSection(sc,"Lighting",y);y=buildToggle(sc,"Full Bright","FullBright",y,function(v) applyFullBright(v) end);y=buildToggle(sc,"No Fog","NoFog",y,function(v) applyNoFog(v) end)
    y=buildSection(sc,"Camera",y+4);y=buildToggle(sc,"FOV Changer","FOVChanger",y,function(v) if not v then Camera.FieldOfView=origCamFOV end end);y=buildSlider(sc,"FOV Value","FOVChangerVal",y,50,200,nil,function(v) if _G.FOVChanger then Camera.FieldOfView=v end end)
    y=buildToggle(sc,"Third Person","ThirdPerson",y,function(v) if v then enableThirdPerson() else disableThirdPerson() end end);y=buildSlider(sc,"Third Person Distance","ThirdPersonDist",y,4,30,nil)
    y=buildSection(sc,"World",y+4);y=buildToggle(sc,"Time Changer","TimeChanger",y,function(v) applyTime(v) end);y=buildSlider(sc,"Time of Day (0-23)","TimeOfDay",y,0,23,nil,function(v) if _G.TimeChanger then applyTime(true) end end)
    y=buildSection(sc,"World Color",y+4);y=buildToggle(sc,"World Color Override","WorldColor",y,function(v) applyWorldColor(v) end);y=buildColorPreview(sc,"WorldR","WorldG","WorldB",y);y=buildSlider(sc,"Red","WorldR",y,0,255,nil,function() if _G.WorldColor then applyWorldColor(true) end end);y=buildSlider(sc,"Green","WorldG",y,0,255,nil,function() if _G.WorldColor then applyWorldColor(true) end end);y=buildSlider(sc,"Blue","WorldB",y,0,255,nil,function() if _G.WorldColor then applyWorldColor(true) end end)
end
tabBuilders["CONFIG"]=function(p)
    local sc=makeScroll(p);local y=10;y=buildSection(sc,"Save Config",y)
    local nameBox;nameBox,y=buildInput(sc,"Config Name","my-pvp-config","",y);local saveBtn;saveBtn,y=buildButton(sc,"Save Config",y,T.KeyGreen,Color3.new(1,1,1))
    local res=Instance.new("TextLabel",sc);res.Size=UDim2.new(1,-28,0,24);res.Position=UDim2.new(0,14,0,y);res.BackgroundTransparency=1;res.Font=Enum.Font.GothamMedium;res.TextSize=13;res.TextXAlignment=Enum.TextXAlignment.Left;y=y+30
    saveBtn.MouseButton1Click:Connect(function() local n=nameBox.Text:gsub("%s+","-");if n=="" then res.Text="Enter a name!";res.TextColor3=T.KeyRed;return end;local ok=saveConfig(n);res.Text=ok and ("Saved: "..n) or "Save failed";res.TextColor3=ok and T.KeyGreen or T.KeyRed;task.wait(0.5);switchTab("CONFIG") end)
    y=buildSection(sc,"Import by Code",y+4)
    local codeBox;codeBox,y=buildInput(sc,"Config Code","SUSANO-CFG-myconfig","",y);local impBtn;impBtn,y=buildButton(sc,"Import Config",y,T.AccentFaint,T.Text)
    local impRes=Instance.new("TextLabel",sc);impRes.Size=UDim2.new(1,-28,0,24);impRes.Position=UDim2.new(0,14,0,y);impRes.BackgroundTransparency=1;impRes.Font=Enum.Font.GothamMedium;impRes.TextSize=13;impRes.TextXAlignment=Enum.TextXAlignment.Left;y=y+30
    impBtn.MouseButton1Click:Connect(function() local name=codeBox.Text:match("^SUSANO%-CFG%-(.+)$");if name then local ok,msg=loadConfig(name);impRes.Text=msg;impRes.TextColor3=ok and T.KeyGreen or T.KeyRed;if ok then buildCrosshair() end else impRes.Text="Invalid code";impRes.TextColor3=T.KeyRed end end)
    y=buildSection(sc,"Saved Configs",y+4)
    local configs=getAllConfigs()
    if #configs==0 then local el=Instance.new("TextLabel",sc);el.Size=UDim2.new(1,-28,0,32);el.Position=UDim2.new(0,14,0,y);el.BackgroundTransparency=1;el.Text="No configs saved yet.";el.TextColor3=T.TextFaint;el.Font=Enum.Font.GothamMedium;el.TextSize=13;el.TextXAlignment=Enum.TextXAlignment.Left
    else
        for _,cfgName in ipairs(configs) do
            ...
            local nl=Instance.new("TextLabel",row);nl.Size=UDim2.new(0.5,0,1,0);nl.Position=UDim2.new(0,12,0,0);nl.BackgroundTransparency=1;nl.Text=cfgName;nl.TextColor3=T.Text;nl.Font=Enum.Font.GothamSemibold;nl.TextSize=14;nl.TextXAlignment=Enum.TextXAlignment.Left
            local function mkBtn(txt,col,xOff) local b=Instance.new("TextButton",row);b.Size=UDim2.new(0,58,0,28);b.Position=UDim2.new(1,xOff,0.5,-14);b.BackgroundColor3=col;b.TextColor3=Color3.new(1,1,1);b.Font=Enum.Font.GothamBold;b.TextSize=12;b.Text=txt;corner(6,b);return b end
            local loadBtn=mkBtn("Load",T.KeyGreen,-164);local codeBtn=mkBtn("Code",T.AccentFaint,-98);local delBtn=mkBtn("Del",T.KeyRed,-32)
            loadBtn.MouseButton1Click:Connect(function() local ok,msg=loadConfig(cfgName);if ok then buildCrosshair() end;res.Text=msg or "Loaded";res.TextColor3=ok and T.KeyGreen or T.KeyRed end)
            codeBtn.MouseButton1Click:Connect(function() res.Text="Code: SUSANO-CFG-"..cfgName;res.TextColor3=T.TextDim end)
            delBtn.MouseButton1Click:Connect(function() deleteConfig(cfgName);res.Text="Deleted: "..cfgName;res.TextColor3=T.KeyRed;task.wait(0.3);switchTab("CONFIG") end)
            y=y+56
        end
    end
end
tabBuilders["ABOUT"]=function(p)
    ...
    local y=104
    local rows={{"Hotkey","F5 — Toggle Menu"},{"Key System","Opens INSTANTLY on load. F5 blocked until validated.\nDaily/Weekly/Monthly/Lifetime. Key saved to device."},{"Respawn","Fly, Noclip, Speed etc. survive death — auto reapplied."},{"OBS Bypass","DisplayOrder 200 hides GUI from OBS Game Capture."},{"Config","Save/Load/Delete configs. Share via SUSANO-CFG-<name> code."}}
    for _,row in ipairs(rows) do local card=Instance.new("Frame",sc);card.Size=UDim2.new(1,-28,0,0);card.AutomaticSize=Enum.AutomaticSize.Y;card.Position=UDim2.new(0,14,0,y);card.BackgroundColor3=T.Card;corner(8,card);local tl=Instance.new("TextLabel",card);tl.Size=UDim2.new(1,0,0,20);tl.Position=UDim2.new(0,12,0,8);tl.BackgroundTransparency=1;tl.Text=string.upper(row[1]);tl.TextColor3=T.TextFaint;tl.Font=Enum.Font.GothamBold;tl.TextSize=10;tl.TextXAlignment=Enum.TextXAlignment.Left;local bl=Instance.new("TextLabel",card);bl.Size=UDim2.new(1,-24,0,0);bl.Position=UDim2.new(0,12,0,26);bl.AutomaticSize=Enum.AutomaticSize.Y;bl.BackgroundTransparency=1;bl.Text=row[2];bl.TextColor3=T.Text;bl.Font=Enum.Font.Gotham;bl.TextSize=13;bl.TextWrapped=true;bl.TextXAlignment=Enum.TextXAlignment.Left;local fp=Instance.new("UIPadding",card);fp.PaddingBottom=UDim.new(0,10);y=y+76 end
end
...
local TABS={{id="ESP",icon="[E]",label="ESP"},{id="AIMBOT",icon="[A]",label="Aimbot"},{id="OTHER",icon="[M]",label="Movement"},{id="PLAYERS",icon="[P]",label="Players"},{id="ITEM",icon="[I]",label="Items"},{id="TEAM CHANGE",icon="[T]",label="Teams"},{id="VISUAL",icon="[V]",label="Visual"},{id="CONFIG",icon="[C]",label="Config"},{id="ABOUT",icon="[?]",label="About"}}
local sideButtons={}
...
switchTab=function(id)
    ...
    for _,btn in ipairs(sideButtons) do
        ...
        if il then tw(il,0.15,{TextColor3=active and T.BG or T.TextFaint}) end
        if nl then tw(nl,0.15,{TextColor3=active and T.BG or T.TextDim}) end
    end
...
local function createMenu()
    -- Create with OBS bypass
    MainGui=makeGui("SusanoUI",200)
    ...
    -- Key badge
    local kBadge=Instance.new("Frame",tb);kBadge.Size=UDim2.new(0,72,0,22);kBadge.Position=UDim2.new(0,156,0.5,-11);kBadge.BackgroundColor3=keyType=="lifetime" and T.KeyGold or (keyType=="monthly" and Color3.fromRGB(180,120,255) or (keyType=="weekly" and Color3.fromRGB(80,180,255) or T.AccentFaint));corner(5,kBadge)
    local kBL=Instance.new("TextLabel",kBadge);kBL.Size=UDim2.new(1,0,1,0);kBL.BackgroundTransparency=1;kBL.Text=keyType:upper();kBL.TextColor3=Color3.new(1,1,1);kBL.Font=Enum.Font.GothamBold;kBL.TextSize=11

    local hotkeyTxt=Instance.new("TextLabel",tb);hotkeyTxt.Size=UDim2.new(0,100,1,0);hotkeyTxt.Position=UDim2.new(0.5,-50,0,0);hotkeyTxt.BackgroundTransparency=1;hotkeyTxt.Text="F5 to toggle";hotkeyTxt.TextColor3=T.TextFaint;hotkeyTxt.Font=Enum.Font.GothamMedium;hotkeyTxt.TextSize=11
    ...
    for i,tab in ipairs(TABS) do
        ...
        local iconL=Instance.new("TextLabel",btn);iconL.Name="IconLbl";iconL.Size=UDim2.new(0,30,1,0);iconL.Position=UDim2.new(0,6,0,0);iconL.BackgroundTransparency=1;iconL.Text=tab.icon;iconL.TextColor3=tab.id==currentTab and T.BG or T.TextFaint;iconL.Font=Enum.Font.GothamBold;iconL.TextSize=10
        local nameL=Instance.new("TextLabel",btn);nameL.Name="NameLbl";nameL.Size=UDim2.new(1,-38,1,0);nameL.Position=UDim2.new(0,34,0,0);nameL.BackgroundTransparency=1;nameL.Text=tab.label;nameL.TextColor3=tab.id==currentTab and T.BG or T.TextDim;nameL.Font=Enum.Font.GothamSemibold;nameL.TextSize=13;nameL.TextXAlignment=Enum.TextXAlignment.Left
        btn.MouseEnter:Connect(function() if btn:GetAttribute("tabId")~=currentTab then tw(btn,0.1,{BackgroundColor3=T.CardHov}) end end)
        btn.MouseLeave:Connect(function() if btn:GetAttribute("tabId")~=currentTab then tw(btn,0.1,{BackgroundColor3=T.InactiveSide}) end end)
...
-- ========== F5 HOTKEY — STRICTLY BLOCKED UNTIL VALIDATED ==========
UserInputService.InputBegan:Connect(function(input,gpe)
    if gpe then return end
    if input.KeyCode==Enum.KeyCode.F5 then
        -- HARD BLOCK: do nothing if not validated
...
-- =====================================================
-- ===== ENTRY POINT — KEY MENU FIRST, INSTANT ======
-- =====================================================
-- Check for saved key FIRST (synchronous, no wait)
local savedKey = loadSavedKey()
if savedKey then
    local ok, ktype = validateKey(savedKey)
    if ok then
        -- Auto-login with saved key — still show key menu briefly to confirm identity
        keyValidated=true; keyType=ktype; _G.Verified=true
        keyMenuGui:Destroy() -- destroy the pre-created GUI since we don't need it
        -- Small delay then open menu
        task.spawn(function()
            task.wait(0.1)
            createMenu()
        end)
        print("[Susano] Auto-login: "..ktype:upper().." key | "..USERNAME)
    else
        -- Saved key expired/invalid, show key menu
        buildKeyMenu(function(ktype2)
            createFOVCircle()
            task.spawn(function() task.wait(0.1); createMenu() end)
        end)
    end
else
    -- No saved key, show key menu immediately
    buildKeyMenu(function(ktype2)
        createFOVCircle()
        task.spawn(function() task.wait(0.1); createMenu() end)
    end)
...
print("================================")
print("  Susano V1.0  |  Full Edition ")
print("  Key required to open menu    ")
