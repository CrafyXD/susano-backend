...
-- MENU SYSTEM & INITIALIZATION (Devam)

-- GUI Parent Düzeltmesi (ESC menüsü üstünde olması için)
pcall(function()
    if syn and syn.protect_gui then
        -- Synapse destekliyorsa koruma altına al
    elseif gethui then
        GuiParent = gethui()
    else
        -- CoreGui erişimi varsa oraya al, yoksa PlayerGui'de kalsın ama DisplayOrder yüksek olsun
        pcall(function() GuiParent = game:GetService("CoreGui") end)
    end
end)

-- Player List (Tüm oyunlarda çalışması için optimize edildi)
local function getPlayers()
    local list = {}
    -- GameSpecific: Bazı oyunlar farklı klasörlerde tutar
    local locations = {Players, Workspace:FindFirstChild("Players"), Workspace:FindFirstChild("Alive"), Workspace:FindFirstChild("Fighters")}
    for _, loc in ipairs(locations) do
        if loc then
            for _, obj in ipairs(loc:GetChildren()) do
                if obj:IsA("Model") or obj:IsA("Player") then
                    -- Eğer bu bir modelse ve içinde Humanoid varsa
    ...
    -- Standart oyuncu listesi her zaman eklensin
    for _, p in ipairs(Players:GetPlayers()) do list[p] = p.Name end
    return list
end
...
-- BUILD MENU
local currentTab = "tab_aim" -- Varsayılan Aimbot sekmesi
local menuOpen = true
...
createMenu = function()
        ...
        KBtn.MouseButton1Click:Connect(function()
            ...
            validateKey(KBox.Text, function(valid, typ, expires)
                if valid then
                    ...
                    keyType = typ
                    keyExpires = expires
                    if KBox.Text ~= "" then safeWrite(KEY_FILE, KBox.Text) end
                    createMenu() -- Rebuild
                else
    ...
    Head.BackgroundColor3 = Tc.TitleBar
    Head.BorderSizePixel = 0
    makeCorner(12, Head)
    local headFix = Instance.new("Frame", Head)
    headFix.Size = UDim2.new(1, 0, 0.5, 0); headFix.Position = UDim2.new(0, 0, 1, -10)
    headFix.BackgroundColor3 = Tc.TitleBar; headFix.BorderSizePixel = 0

    local HTitle = Instance.new("TextLabel", Head)
    HTitle.Size = UDim2.new(0, 200, 1, 0)
    HTitle.Position = UDim2.new(0, 15, 0, 0)
    HTitle.BackgroundTransparency = 1
    HTitle.Text = "SUSANO V2.2 | " .. (keyType or "USER")
    HTitle.TextColor3 = Tc.Accent
    ...
    TimeL.Size = UDim2.new(0, 120, 1, 0)
    TimeL.Position = UDim2.new(1, -130, 0, 0)
    TimeL.BackgroundTransparency = 1
    TimeL.Text = formatTimeLeft(keyExpires)
    TimeL.TextColor3 = Tc.KeyGold
    TimeL.Font = Enum.Font.GothamBold
    TimeL.TextSize = 12
    -- Timer Update
    task.spawn(function()
        while MainGui and keyValidated do
            task.wait(1)
            if TimeL and TimeL.Parent then
                TimeL.Text = formatTimeLeft(keyExpires)
            else break end
        end
    end)
    ...
    Side.BackgroundColor3 = Tc.Sidebar
    ...
    for _, tKey in ipairs(tabsOrder) do
        ...
        Btn.BackgroundColor3 = (currentTab == tKey) and Tc.Card or Tc.Sidebar
        Btn.TextColor3 = (currentTab == tKey) and Tc.Accent or Tc.TextDim
        Btn.Text = tName
    ...
    elseif currentTab == "tab_vis" then
        ...
        y = buildToggle(Content, T("t_fv"), "FOVChanger", y, function(v) 
            if v then pcall(function() Camera.FieldOfView = _G.FOVChangerVal end)
    ...
    elseif currentTab == "tab_play" then
        ...
        local nameBox, y2 = buildInput(Content, T("l_nm"), "Fake Name...", _G.SpoofedName, y)
        nameBox.FocusLost:Connect(function() _G.SpoofedName = nameBox.Text; applyNameSpoof() end)
        y = y2
        
        y = buildSection(Content, "Oyuncu Listesi", y)
        -- Basit bir liste, butonlara gerek yok, sadece ESP vs çalışır
        -- Player list logic can be complex, simplified here
        local listLabel = Instance.new("TextLabel", Content)
        listLabel.Size = UDim2.new(1, -28, 0, 100)
        listLabel.Position = UDim2.new(0, 14, 0, y)
        listLabel.BackgroundTransparency = 1
        listLabel.TextColor3 = Tc.TextDim
        listLabel.TextSize = 12
        listLabel.Font = Enum.Font.Gotham
        listLabel.TextXAlignment = Enum.TextXAlignment.Left
        listLabel.TextYAlignment = Enum.TextYAlignment.Top
        listLabel.Text = "Liste yukleniyor..."
        task.spawn(function()
            while listLabel and listLabel.Parent do
                local pList = getPlayers()
                local txt = ""
                for p, name in pairs(pList) do
                    if p ~= LocalPlayer then
                        txt = txt .. name .. "\n"
                    end
                end
                listLabel.Text = txt ~= "" and txt or "Oyuncu bulunamadi"
                task.wait(1)
            end
        end)
    ...
    elseif currentTab == "tab_ac" then
        ...
        y = buildToggle(Content, T("t_acre"), "ACBlockRemotes", y, function(v) 
            if v then 
                setupAntiCheatBypass() 
            else 
                -- Remotes blocked oldugu icin geri acmak zordur, uyar ver
                print("Remotes block kaldirilamaz, tekrar giris gerekebilir.")
            end 
        end)
    ...
    elseif currentTab == "tab_cfg" then
        ...
        local cfgNameBox, y2 = buildInput(Content, "Config Name", "Isim...", "", y)
        y = y2
        
        local saveBtn, y3 = buildButton(Content, T("cfg_save"), y, Tc.KeyGreen, Color3.new(1,1,1))
        ...
        local loadBtn, y4 = buildButton(Content, T("cfg_load"), y, Tc.Accent, Color3.new(1,1,1))
        ...
        if #configs == 0 then
            local noCfg = Instance.new("TextLabel", Content)
            noCfg.Size = UDim2.new(1, -28, 0, 20)
            noCfg.Position = UDim2.new(0, 14, 0, y)
            noCfg.BackgroundTransparency = 1
            noCfg.TextColor3 = Tc.TextDim
            noCfg.Text = T("cfg_none")
            y = y + 24
        else
            for _, cfg in ipairs(configs) do
                local cfgBtn = Instance.new("TextButton", Content)
                cfgBtn.Size = UDim2.new(1, -28, 0, 30)
                cfgBtn.Position = UDim2.new(0, 14, 0, y)
                cfgBtn.BackgroundColor3 = Tc.Card
                cfgBtn.TextColor3 = Tc.Text
                cfgBtn.Text = cfg
...
local function init()
    ...
    if savedKey then
        validateKey(savedKey, function(valid, typ, expires)
            if valid then
                keyValidated = true
                keyType = typ
                keyExpires = expires
    ...
    -- Setup Systems
    setupAutoRejoin()
    enableTeleportCursor() -- TP sistemini baslat
    createFOVCircle()
    
    -- Loops
    RunService.Heartbeat:Connect(function()
        -- ESP Logic
        if _G.ESP then updateSkeleton() end
        -- Theme Rainbow
